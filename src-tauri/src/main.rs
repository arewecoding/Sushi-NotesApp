...
use sushi_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m sushi`,
    // i.e, run the `src-tauri/python/sushi/__main__.py`
    let py_script = PythonScript::Module("sushi".into());
